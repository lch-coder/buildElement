...
<template>
  <div>
    <div title="高级表单" desc="高级表单常见于一次性输入和提交大批量数据的场景。" />
        ...
        <el-table :data="form.users">
          <el-table-column label="成员姓名">
            <template #default="{ row }">
              <el-input
                v-if="editUserMap[row.id]"
                :key="`input${row.id}`"
                v-model="editUserMap[row.id].name"
                autofocus
              />
              <template v-else>
                {{ row.name }}
          ...
          <el-table-column label="操作">
            <template #default="{ $index, row }">
              <div class="space-x-2">
                <el-link v-if="!editUserMap[row.id]" type="primary" @click="showUserEdit(row)"> 编辑 </el-link>

                <template v-else>
                  <el-link type="primary" @click="saveUserEdit(row)"> 保存 </el-link>
                  <el-popconfirm title="确认删除?" @confirm="deleteUser($index)">
                    <template #reference>
                      <el-link type="primary"> 删除 </el-link>
                    </template>
                  </el-popconfirm>
                  <el-link v-if="editUserMap[row.id]" type="primary" @click="cancelUserEdit(row)"> 取消 </el-link>
                </template>
              </div>
            </template>
          </el-table-column>
